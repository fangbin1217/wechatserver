...
class Controller extends \yii\base\Controller
{
    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            ...
            //是否免登录
            if (in_array($action->controller->id, Yii::$app->params['withoutlogin'])) {
                return true;
            } else {
                $params = json_decode(file_get_contents('php://input'),true);
                $access_token = $params['access_token'] ?? '';
                //check login
                if(Users::isLogin($access_token)) {
                    return true;
                }
            }
            //return false;
        ...
        $this->jsonResponse['msg'] = 'access_token is expired, please login again';
        Yii::$app->response->data = $this->jsonResponse;
        return false;
    }
